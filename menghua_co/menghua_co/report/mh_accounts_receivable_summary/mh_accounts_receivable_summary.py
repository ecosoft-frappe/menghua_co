...
class AccountsReceivableSummary(ReceivablePayableReport):
    ...
    def get_data(self, args):
        ...
        invoice_nos = list({
            ...
            if r.get("voucher_type") == "Sales Invoice" and r.get("voucher_no")
        })
        ...
        if invoice_nos:
            sales_team_rows = frappe.db.get_all(
                "Sales Team",
                filters={"parenttype": "Sales Invoice", "parent": ("in", invoice_nos)},
                fields=["parent", "sales_person"],
                order_by="parent asc, idx asc",
            )
            for d in sales_team_rows:
                sales_person_map.setdefault(d.parent, []).append(d.sales_person)

        for row in self.receivables:
            row.sales_person = ""
            if (
                row.get("voucher_type") == "Sales Invoice"
                and row.get("voucher_no") in sales_person_map
            ):
                row.sales_person = ", ".join(
                    sorted(set(sales_person_map[row.voucher_no]))
                )
        ...
        for party, party_dict in self.party_total.items():
            ...
            if isinstance(row.sales_person, list):
                uniq = []
                seen = set()
                for sp in row.sales_person:
                    for name in [n.strip() for n in sp.split(",") if n.strip()]:
                        if name not in seen:
                            seen.add(name)
                            uniq.append(name)
                row.sales_person = ", ".join(uniq)
    ...
    def init_party_total(self, row):
        ...
        self.party_total.setdefault(row.party, frappe._dict(default_dict))
    ...
    def set_party_details(self, row):
        ...
        if row.sales_person and row.sales_person not in self.party_total[row.party].sales_person:
...
def get_gl_balance(report_date, company):
    return frappe._dict(
        frappe.db.get_all(
            "GL Entry",
            fields=["party", "sum(debit) - sum(credit) as balance"],
